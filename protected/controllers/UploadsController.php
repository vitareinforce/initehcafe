...
if(isset($_POST['Uploads']))
{
	...
	if($model->save()) {
		$uploadedFile->saveAs(Yii::getPathOfAlias('webroot') . '/images/uploads/'.$fileName);
...
if(isset($_POST['Uploads']))
{
	...
	$rnd = rand(0,9999);
	$_POST['Uploads']['image'] = $model->image;
	$model->attributes=$_POST['Uploads'];                      
	$uploadedFile=CUploadedFile::getInstance($model,'image');
	if($uploadedFile != null) {
	    $fileName = "{$rnd}_{$uploadedFile}";  // random number + file name
    ...
    if($model->save()) {
    	...
    	{
    		$uploadedFile->saveAs(Yii::getPathOfAlias('webroot') . '/images/uploads/'.$fileName);
    		unlink(Yii::getPathOfAlias('webroot') . '/images/uploads/'.$oldfilename);
    	}
		$this->redirect(array('view','id'=>$model->id));
	}
}
